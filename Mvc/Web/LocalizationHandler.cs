...
namespace Knoema.Localization.Web
{
	public class LocalizationHandler : IHttpHandler
	{
		...
		public static string RenderIncludes(bool admin, List<string> scope)
		{
			...
			var names = GetResourceAssembly().GetManifestResourceNames();
		...
		{
			var path = GetResourceAssembly().GetName().Name + ".Resources.";
		...
		{
			return typeof(ILocalizationProvider).Assembly;
		}

		private static Stream GetResourceStream(string path)
		{
			return GetResourceAssembly().GetManifestResourceStream(path);
		}
